...
RoomPlan scan import now has a separate deterministic strict-safe normalization policy documented in
`docs/qa/roomplan-import-validation.md`. It does not alter the UUID handoff:
header uploads remain client-owned and header-less uploads remain legacy until
both applications can share a UUID.
